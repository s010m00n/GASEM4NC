...
def visualize_comparison(h, true_labels, pred_labels):
    ...
    plt.savefig('results/pubmed_class2_comparison.png')
    plt.show()
...
for epoch in range(max_epochs):
    ...
    if epoch % 5 == 0 or epoch == 0:
        ...
        # 计算类别2的准确率
        mask2 = (data.y == 2) & data.test_mask
        if mask2.sum() > 0:
            correct2 = (pred[mask2] == data.y[mask2]).sum()
            class2_acc = float(correct2) / float(mask2.sum())
        else:
            class2_acc = 0
        
        # 使用加权指标来决定最佳模型，特别关注类别2
        weighted_metric = 0.7 * val_acc + 0.3 * class2_acc
        ...
        if weighted_metric > best_val_acc:
            ...
            # 保存最佳模型
            torch.save(model.state_dict(), 'results/best_attn_model_class2.pt')
        else:
            no_improve_count += 1
    ...
    if epoch % 10 == 0:
        ...
        if epoch % 100 == 0 and epoch > 0:
            torch.save(model.state_dict(), f'results/multihop_gat_model_class2_epoch_{epoch}.pt')
...
# 如果保存了最佳模型，加载它
if os.path.exists('results/best_attn_model_class2.pt'):
    model.load_state_dict(torch.load('results/best_attn_model_class2.pt'))
    print("已加载最佳模型")
...
plt.savefig('results/pubmed_class2_hop_weights.png')
plt.show()
...
plt.ylabel('准确率')
plt.ylim(0, 1)
plt.savefig('results/pubmed_class2_class_accs.png')
plt.show()
...
plt.tight_layout()
plt.savefig('results/pubmed_class2_training.png')
plt.show()

# 保存实验结果
with open('results/experiment_results_pubmed_class2.txt', 'w') as f:
    f.write(f"最佳验证准确率: {best_val_acc:.4f}\n")
    f.write(f"最佳测试准确率: {best_test_acc:.4f}\n")
    f.write(f"类别2准确率: {class2_acc:.4f}\n")
    f.write(f"跳数注意力权重: {hop_weights}\n")
    f.write(f"各类别准确率: {class_accs}\n")
    f.write(f"类别平衡权重: {class_weights.cpu().numpy()}\n")
    f.write(f"加权指标计算方式: 0.7*val_acc + 0.3*class2_acc\n")
    
print("实验结果已保存到 results/experiment_results_pubmed_class2.txt") 