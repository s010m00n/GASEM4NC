...
for epoch in range(3000):  # 增加最大epoch数
    ...
    if epoch % 100 == 0 and epoch > 0:
        torch.save(model.state_dict(), f'results/gnn_model_epoch_{epoch}.pt')
...
# 如果保存了最佳模型，加载它
if os.path.exists('results/best_gnn_model.pt'):
...
# 保存实验结果
os.makedirs("results", exist_ok=True)
with open('results/experiment_results_gnn.txt', 'w') as f:
...
print("实验结果已保存到 results/experiment_results_gnn.txt") 