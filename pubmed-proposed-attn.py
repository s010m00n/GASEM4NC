...
def test():
    ...
    return accs[0], accs[1], accs[2], hop_attn, out, class_accs, class_counts
...
for epoch in range(max_epochs):
    ...
    if epoch % 5 == 0 or epoch == 0:
        train_acc, val_acc, test_acc_current, test_hop_attn, out, class_accs, class_counts = test()
        ...
        # 类别4
        mask4 = (data.y == 4) & data.test_mask
        if mask4.sum() > 0:
            correct4 = (pred[mask4] == data.y[mask4]).sum()
            class4_acc = float(correct4) / float(mask4.sum())
        else:
            class4_acc = 0
        
        # 类别5
        mask5 = (data.y == 5) & data.test_mask
        if mask5.sum() > 0:
            correct5 = (pred[mask5] == data.y[mask5]).sum()
            class5_acc = float(correct5) / float(mask5.sum())
        else:
            class5_acc = 0
        ...
        if weighted_metric > best_val_acc:
            ...
            # 保存最佳模型
            torch.save(model.state_dict(), 'results/best_attn_model.pt')
        else:
            no_improve_count += 1
    ...
    if epoch % 10 == 0:
        ...
        print(f"各类别准确率: 类别0: {class_accs[0]:.4f}, 类别1: {class_accs[1]:.4f}, 类别2: {class_accs[2]:.4f}")
        print(f"各类别样本数: 类别0: {class_counts[0]}, 类别1: {class_counts[1]}, 类别2: {class_counts[2]}")
        ...
        if epoch % 100 == 0 and epoch > 0:
            torch.save(model.state_dict(), f'results/multihop_gat_model_epoch_{epoch}.pt')
...
# 如果保存了最佳模型，加载它
if os.path.exists('results/best_attn_model.pt'):
    model.load_state_dict(torch.load('results/best_attn_model.pt'))
    print("已加载最佳模型")
    _, _, final_test_acc, test_hop_attn, best_out, final_class_accs, final_class_counts = test()
    hop_weights = test_hop_attn.detach().cpu().numpy()
    
    print(f"\n最终测试准确率: {final_test_acc:.4f}")
    print("最终各类别准确率:")
    for c in range(dataset.num_classes):
        print(f"类别 {c}: {final_class_accs[c]:.4f} (样本数: {final_class_counts[c]})")
...
# 保存实验结果
with open('results/experiment_results_multihop.txt', 'w') as f:
    ...
    f.write(f"加权指标计算方式: val_acc\n\n")
    f.write("各类别准确率:\n")
    for c in range(dataset.num_classes):
        f.write(f"类别 {c}: {final_class_accs[c]:.4f} (样本数: {final_class_counts[c]})\n")
    
print("实验结果已保存到 results/experiment_results_multihop.txt") 