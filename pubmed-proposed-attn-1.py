...
def visualize_comparison(h, true_labels, pred_labels):
    ...
    plt.savefig('results/pubmed_class1_comparison.png')
    plt.show()
...
for epoch in range(max_epochs):
    ...
    if epoch % 5 == 0 or epoch == 0:
        ...
        # 计算类别1的准确率
        mask1 = (data.y == 1) & data.test_mask
        if mask1.sum() > 0:
            correct1 = (pred[mask1] == data.y[mask1]).sum()
            class1_acc = float(correct1) / float(mask1.sum())
        else:
            class1_acc = 0
        
        # 使用加权指标来决定最佳模型，特别关注类别1
        weighted_metric = 0.7 * val_acc + 0.3 * class1_acc
        ...
        if weighted_metric > best_val_acc:
            ...
            # 保存最佳模型
            torch.save(model.state_dict(), 'results/best_attn_model_class1.pt')
        else:
            no_improve_count += 1
    ...
    if epoch % 10 == 0:
        ...
        if epoch % 100 == 0 and epoch > 0:
            torch.save(model.state_dict(), f'results/multihop_gat_model_class1_epoch_{epoch}.pt')
...
# 如果保存了最佳模型，加载它
if os.path.exists('results/best_attn_model_class1.pt'):
    model.load_state_dict(torch.load('results/best_attn_model_class1.pt'))
    print("已加载最佳模型")
...
plt.savefig('results/pubmed_class1_hop_weights.png')
plt.show()
...
plt.ylabel('准确率')
plt.ylim(0, 1)
plt.savefig('results/pubmed_class1_class_accs.png')
plt.show()
...
plt.tight_layout()
plt.savefig('results/pubmed_class1_training.png')
plt.show()

# 保存实验结果
with open('results/experiment_results_pubmed_class1.txt', 'w') as f:
    f.write(f"最佳验证准确率: {best_val_acc:.4f}\n")
    f.write(f"最佳测试准确率: {best_test_acc:.4f}\n")
    f.write(f"类别1准确率: {class1_acc:.4f}\n")
    f.write(f"跳数注意力权重: {hop_weights}\n")
    f.write(f"各类别准确率: {class_accs}\n")
    f.write(f"类别平衡权重: {class_weights.cpu().numpy()}\n")
    f.write(f"加权指标计算方式: 0.7*val_acc + 0.3*class1_acc\n")
    
print("实验结果已保存到 results/experiment_results_pubmed_class1.txt") 